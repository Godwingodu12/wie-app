...
const ChangeContact = () => {
  ...
  const handleSendOTP = async () => {
    ...
    setLoading(true)
    try {
      await authService.forgotPasswordSendOtp(input)
      router.push({
        pathname: '/(public)/ForgotPassword/VerifyOtp',
        params: { emailOrPhone: input }
      })
    } catch (error: any) {
    ...
    <SafeAreaView className="flex-1 bg-black">
      ...
      >
        <View className="items-center">
          ...
          <Text className="text-white font-rubik-extrabold text-3xl mt-4 text-center">
          ...
          <View className="w-full mt-10">
            <InputFiled
              placeholder="New Email or Phone number"
              value={emailOrPhone}
