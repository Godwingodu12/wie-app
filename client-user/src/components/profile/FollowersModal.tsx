...
import { getFollowers, followUser, unfollowUser, checkFollowStatus } from "@/services/followService";
interface Follower {
  id: string;
  name: string;
  username: string;
...
const fetchFollowers = async () => {
  try {
    ...
    if (isOwnProfile) {
      ...
      const mappedFollowers = (data.followers || []).map((f: any) => ({
        ...f,
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-[#2D2F39]">
            <h2 className="text-xl font-bold text-white">
              {isOwnProfile ? 'Followers' : `${userName || 'User'}'s Followers`}
            </h2>
            <button
              onClick={onClose}
