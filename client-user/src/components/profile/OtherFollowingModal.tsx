...
import { getOtherFollowing, followUser, unfollowUser } from "@/services/followService";
...
interface OtherFollowingModalProps {
  isOpen: boolean;
  onClose: () => void;
  userId: string;
  userName?: string;
...
export default function OtherFollowingModal({
  isOpen,
  onClose,
  userId,
  userName,
  onFollowingCountChange
}: OtherFollowingModalProps) {
  ...
  const fetchFollowing = async () => {
    try {
      setLoading(true);
      const data = await getOtherFollowing(userId);
  ...
  const handleFollowToggle = async (followingId: string, isCurrentlyFollowing: boolean) => {
    try {
      setActionLoadingId(followingId);
      ...
      setFollowing(prev =>
        prev.map(u =>
          u.id === followingId ? { ...u, isFollowing: !isCurrentlyFollowing } : u
        )
      );
    } catch (error) {
