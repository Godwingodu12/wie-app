...
export default function ChatList({ onChatSelect }: ChatListProps) {
  ...
  useEffect(() => {
    ...
    const handleNewChatAdded = (event: CustomEvent) => {
      ...
      if (chat) {
        // Switch to Personal tab to show the new chat
  ...
  const handleChatCreated = (chatId: string, chat: any) => {
    try {
      ...
      // ✅ FIX: Switch to appropriate tab when creating new chat
      if (chatObject.type === 'group') {
  ...
  if (isRefreshing && chats.length === 0) {
    return renderLoader();
  }
  ...
  return (
    <div className="flex flex-col h-full w-full" style={{ backgroundColor: themeStyles.sidebarBg, color: themeStyles.text }}>
      ...
      {/* List - Render appropriate tab component */}
