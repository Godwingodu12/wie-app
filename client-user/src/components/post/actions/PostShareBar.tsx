...
const SOCIAL_APPS = [
  {
    ...
    action: (url: string) => { navigator.clipboard.writeText(url).catch(() => {}); },
  },
  {
    label: "WhatsApp",
  ...
  },
  {
    ...
    action: (_url: string) => window.open("https://www.instagram.com/", "_blank"),
  },
  {
    label: "Facebook",
...
export default function PostShareBar({ onClose, postId, postOwnerId }: PostShareBarProps) {
  ...
  const postUrl = typeof window !== "undefined"
    ? `${window.location.origin}/post/${postId}${postOwnerId ? `?userId=${postOwnerId}` : ""}`
    : "";
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await sharePost(postId, selected);

      // Force chat list reload so sender sees shared post in chat
      ...
      // Show success anyway — may have been queued
      setSent(true);
      setTimeout(onClose, 1100);
    } finally {
      setSending(false);
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <span style={{ color: "#fff", fontSize: 15, fontWeight: 600 }}>Share Post</span>
      ...
      {/* Bottom: social row OR send button */}
      {selected.length === 0 ? (
        <div style={{
