...
import { createNotification, emitMentionEvent } from '../utils/notificationHelper';
import * as chatClient from '../grpc/clients/chatClient';
export const createFlux = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.file) {
      res.status(400).json({ success: false, message: 'Media file is required' });
      return;
    }
    const flux = await fluxService.createFlux(req.userId!, req.file, req.body);
...
export const getFluxFeed = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
// GET /flux/:fluxId
export const getFluxById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
export const getUserFluxes = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.json({
      ...
      data:    fluxes,
      count:   fluxes.length,
      message: fluxes.length === 0 ? 'No active fluxes for this user' : undefined,
    });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
...
export const viewFlux = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { fluxId } = req.params;
...
export const reactToFlux = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { fluxId } = req.params;
...
export const getFluxViewers = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { fluxId } = req.params;
...
export const deleteFlux = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { fluxId } = req.params;
...
export const getAllMyFluxes = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
export const getArchivedFluxes = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
 ...
 */
export const getMyFluxes = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
 ...
 */
export const getViewers = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { fluxId } = req.params;
    const ownerId    = req.userId!;

    const flux = await FluxModel
      .findOne({ _id: fluxId, userId: ownerId, isDeleted: false })
      .select('viewers')
      .lean();

    if (!flux) {
      res.status(404).json({ success: false, message: 'Flux not found or not yours' });
      return;
    }
    ...
    res.status(500).json({ success: false, message: err.message ?? 'Failed to fetch viewers' });
  }
};
...
export const updateFluxVisibility = async (
  ...
  try {
    ...
    const allowed: FluxVisibility[] = ['public', 'followers', 'close_friends', 'only_me'];
    ...
    res.json({ success: true, data: { _id: flux._id, visibility: flux.visibility } });
  } catch (err: any) {
...
export const hideFluxFromUser = async (
  ...
  try {
    ...
    if (!hideUserId) {
      res.status(400).json({ success: false, message: 'hideUserId is required' });
      return;
    }
...
export const unhideFluxFromUser = async (
  ...
  try {
    ...
    if (!unhideUserId) {
      res.status(400).json({ success: false, message: 'unhideUserId is required' });
      return;
    }
...
export const mentionFlux = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const callerId: string | undefined = req.userId?.toString();
    const { fluxId } = req.params;

    const mentionedUserIds: string[] | undefined = Array.isArray((req as any).body?.mentionedUserIds)
      ? (req as any).body.mentionedUserIds
      : undefined;

    if (!callerId) {
      console.error('[mentionFlux] No callerId found. req.userId =', req.userId);
      res.status(401).json({ success: false, message: 'Unauthorized' });
      return;
    }
    if (!mentionedUserIds || mentionedUserIds.length === 0) {
      res.status(400).json({ success: false, message: 'mentionedUserIds must be a non-empty array' });
      return;
    }
    ...
    if (flux.userId.toString() !== callerId) {
      res.status(403).json({ success: false, message: 'You can only add mentions to your own flux' });
      return;
    }
    ...
    for (const targetId of mentionedUserIds) {
      // Skip self
      if (targetId.toString() === callerId) { rejected.push(targetId); continue; }

      // Skip already mentioned (same flux, same user — one mention per user per flux)
      if (alreadyMentionedIds.has(targetId.toString())) { duplicates.push(targetId); continue; }

      // Must be following the target
      const followResp = await isFollowing(callerId, targetId).catch(() => ({ isFollowing: false }));
      if (!followResp.isFollowing) { rejected.push(targetId); continue; }

      // Block check
      const blockResp = await wieUserClient.checkIfBlocked(callerId, targetId).catch(() => ({ blocked: false }));
      if (blockResp.blocked) { rejected.push(targetId); continue; }

      // Privacy check — if target has a private account, skip unless they follow back
      const targetPrivacy = await wieUserClient.getAccountPrivacy(targetId).catch(() => null);
      if (targetPrivacy?.accountPrivacy === 'private') {
        // Target is private — only allow mention if target follows the caller back (mutual)
        const mutualResp = await isFollowing(targetId, callerId).catch(() => ({ isFollowing: false }));
    ...
    // Fetch user details
    const usersResp = await wieUserClient.getUsersByIds(allowed).catch(() => ({ users: [] }));
    const callerResp = await wieUserClient.getUsersByIds([callerId]).catch(() => ({ users: [] }));
    ...
    const callerAvatar = callerInfo?.profile_picture ?? callerInfo?.profilePicture ?? '';
    ...
    for (const u of (usersResp.users ?? [])) {
      ...
      try {
        ...
        console.log(`✅ Chat message sent: chatId=${chatResult.chat_id}, msgId=${chatResult.message_id}`);
      } catch (err) {
    ...
    res.status(200).json({
      ...
      ...(rejected.length > 0   && { rejected,   warning: 'Some users skipped (not following or blocked)' }),
      ...(duplicates.length > 0 && { duplicates, info: 'Some users were already mentioned' }),
    });
  } catch (err) {
...
export const reMentionFlux = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const callerId = req.userId?.toString();
    const { fluxId } = req.params;
    ...
    if (flux.userId.toString() === callerId) {
      res.status(400).json({ success: false, message: 'Cannot remention your own flux' });
      return;
    }
    ...
    if (!isMentioned) {
      res.status(403).json({ success: false, message: 'You are not mentioned in this flux' });
      return;
    }
    ...
    if (alreadyReMentioned) {
      res.status(409).json({ success: false, message: 'You have already re-mentioned this flux' });
      return;
    }
    ...
    try {
      ...
      console.log(`✅ Remention notification sent to flux owner ${flux.userId}`);
    } catch (err) {
    ...
    try {
      ...
      console.log(`✅ Remention chat message sent: chatId=${chatResult.chat_id}, msgId=${chatResult.message_id}`);
    } catch (err) {
...
export const getFluxMentions = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const callerId = req.userId?.toString();
    const { fluxId } = req.params;
    ...
    const mentionUserIds = (flux.mentions ?? []).map((m: any) => m.userId?.toString()).filter(Boolean);

    const usersResp = mentionUserIds.length > 0
      ? await wieUserClient.getUsersByIds(mentionUserIds).catch(() => ({ users: [] }))
      : { users: [] };

    const enriched = (flux.mentions ?? []).map((m: any) => {
      const uid  = m.userId?.toString();
      const user = (usersResp.users ?? []).find((u: any) => (u._id ?? u.id)?.toString() === uid);
      return {
        userId:    uid,
    ...
    res.status(200).json({ success: true, mentions: enriched, total: enriched.length });
  } catch (err) {
...
export const getReMentions = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const callerId = req.userId?.toString();
    const { fluxId } = req.params;
    ...
    const userIds = reMentions.map((r: any) => r.userId?.toString()).filter(Boolean);

    const usersResp = userIds.length > 0
