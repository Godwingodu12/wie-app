...
export const getGroupBookings = async (req, res) => {
    try {
        ...
        const { ticketId, status, startDate, endDate, limit = 100, skip = 0, } = req.query;
        const options = {
...
export const getEventStatistics = async (req, res) => {
    try {
        ...
        const recentBookings = bookings.filter((b) => b.bookingStatus === 'CONFIRMED' &&
            new Date(b.createdAt) >= thirtyDaysAgo);
        const dailyBookings = recentBookings.reduce((acc, booking) => {
        ...
        res.json({
            ...
            data: {
                ...
                interactionStats,
                paymentMethods,
                dailyBookings: Object.values(dailyBookings).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
            },
        });
    }
    catch (error) {
...
export const verifyTicketQR = async (req, res) => {
    try {
        ...
        await createNotification({
            ...
            message: `Your ticket for ${booking.eventDetails.eventName} has been verified`,
...
export const exportBookings = async (req, res) => {
    try {
        ...
        let bookings = []; // Fixed: Explicitly type as Booking[]
        if (ticketId) {
            bookings = await BookingModel.findByTicketId(ticketId);
        }
        else if (groupId) {
        ...
        const rows = bookings.map((b) => {
            const userDetails = b.userDetails;
            const eventDetails = b.eventDetails;
            return [
                b.bookingId,
                b.userId,
...
export const getBookingAnalytics = async (req, res) => {
    try {
        ...
        const averageOrderValue = confirmedBookings > 0 ? totalRevenue / confirmedBookings : 0;
        ...
        // Hourly distribution
        const hourlyDistribution = filteredBookings.reduce((acc, b) => {
...
export const getTopEventsByRevenue = async (req, res) => {
    try {
        const { groupId } = req.params;
        ...
        const eventRevenue = result.bookings
            ...
            if (!acc[b.ticketId]) {
                acc[b.ticketId] = {
                    ticketId: b.ticketId,
                    eventName: b.eventDetails?.eventName || 'Unknown',
                    totalRevenue: 0,
                    totalBookings: 0,
                    totalTickets: 0,
                };
