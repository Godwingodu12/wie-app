...
export class SettlementService {
   ...
   */
  static async getOrganizationSettlements(groupId: string, startDate?: Date, endDate?: Date) {
    const where: any = {};
    ...
    for (const settlement of settlements) {
      ...
      await prisma.settlement.update({
        ...
        data: {
          ...
          processedAt: new Date(),         // ✅ correct field name (not settledAt)
        },
      });
    }
...
static async processSettlementJob(settlementId: string): Promise<{
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (updated.count === 0) {
      return { success: false, reason: 'Race condition — already being processed' };
    }
...
static async executeHostTransfer(settlementId: string): Promise<void> {
  ...
  if (!hostAccount || !hostAccount.razorpay_account_id) {
    ...
    throw new Error(`No Razorpay linked account for group ${settlement.groupId}`);
  }
  ...
  try {
    ...
    throw new Error(`Transfer failed (attempt ${retryCount + 1}): ${err.message}`);
  }
};
...
static async handlePostSettlementRefund(data: {
  ...
  try {
    ...
    console.warn(`⚠️ Reversal failed — creating host adjustment for group ${data.groupId}`);
  }
...
static async triggerEventSettlements(ticketId: string): Promise<{
  ...
  // Publish to settlement queue
    await publishSettlementJobs(settlements.map((s) => ({
      settlementId: s.id,
      bookingId:    s.bookingId,
      groupId:      s.groupId!,
      hostAmount:   parseFloat(s.organizationAmount.toString()),
    })));

    return { queued: settlements.length };
  }
}
