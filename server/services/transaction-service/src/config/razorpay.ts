...
class RazorpayService {
  ...
  static async createOrder(
    ...
    notes?: any,
    routeTransfer?: {
      // ← NEW: optional Razorpay Route split
      hostAccountId: string;
      hostAmount: number;
      onHold: 0 | 1; // 1 = escrow, 0 = instant
      onHoldUntil?: number; // Unix timestamp for release
    },
  ): Promise<any> {
    try {
      ...
      // Razorpay Route: split payment at source if host has a linked account
      if (routeTransfer?.hostAccountId) {
        orderPayload.transfers = [
          {
            account: routeTransfer.hostAccountId,
            amount: Math.round(routeTransfer.hostAmount * 100),
            currency,
            on_hold: routeTransfer.onHold,
            ...(routeTransfer.onHoldUntil && {
              on_hold_until: routeTransfer.onHoldUntil,
            }),
            notes: {
              settlement_type: routeTransfer.onHold ? "ESCROW" : "INSTANT",
            },
          },
        ];
      }
  ...
  static async transferToHost(data: {
    ...
    try {
      ...
      console.log(
        `✅ Razorpay transfer created: ${transfer.id} → ₹${data.amount}`,
      );
      return transfer;
    } catch (error: any) {
      console.error(
