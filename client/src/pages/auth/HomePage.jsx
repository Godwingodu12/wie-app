...
import { getGroups, getMyLiveEvents, getMyEvents,groupEventCount } from "../../services/ticketService";
...
const HomePage = () => {
    ...
    const fetchEventStats = async () => {
      ...
      try {
        ...
        allEventsArray.forEach(event => {
          if (event.groupId) {
            groupEventCount[event.groupId] = (groupEventCount[event.groupId] || 0) + 1;
          }
        });
        
        // Create group stats with names
        const groupStats = Object.entries(groupEventCount).map(([groupId, count]) => {
  ...
  const handlePrevGroup = () => {
    setEventStats(prev => ({
      ...prev,
      currentGroupIndex: prev.currentGroupIndex > 0 ? prev.currentGroupIndex - 1 : 0
    }));
  };
  const handleNextGroup = () => {
    setEventStats(prev => ({
      ...prev,
      currentGroupIndex: prev.currentGroupIndex < prev.groupStats.length 
    ...
    <style>{`
      ...
      <div className={`${theme.bg} ${theme.text} min-h-screen flex overflow-hidden transition-colors duration-300`}>
        ...
        <div className="flex flex-col flex-1 relative overflow-hidden">
          ...
          <main className={`main-scrollbar flex flex-col flex-1 p-4 md:px-6 md:pt-6 overflow-y-auto pb-32 md:pb-4 ${theme.cardBg}`}>
                          ...
                          <div className="flex flex-col gap-4">
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {dashboardStats.map((stat) => {
                                  ...
                                  const isShowingTotal = stat.isEventStat && eventStats.currentGroupIndex === 0;
                                  const currentGroupStat = stat.isEventStat && eventStats.currentGroupIndex > 0 
                                    ? eventStats.groupStats[eventStats.currentGroupIndex - 1] 
                                    : null;
                                  
                                  const displayCount = stat.isEventStat 
                                    ? (isShowingTotal ? eventStats.totalCount : currentGroupStat?.count || 0)
                                    : null;
                                    
                                  const displayLabel = stat.isEventStat
                                    ? (isShowingTotal ? "Total Events" : currentGroupStat?.groupName || "")
                                    : null;
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      ></div>
                                      <img src={stat.icon} alt={stat.title} className="w-5 h-5 absolute left-5 top-3 z-20" />
                                      ...
                                      <div className="flex flex-col items-center w-full mt-4">
                                        ...
                                        {stat.isEventStat ? (
                                          <>
                                            {loadingEventStats ? (
                                              <div className={`${theme.subText} text-base text-center mt-4`}>Loading...</div>
                                              ...
                                              <div className="flex flex-col items-center w-full">
                                                <div className={`text-4xl font-bold ${theme.text} mt-2`}>
                                                  {displayCount}
                                                </div>
                                                <div className={`${theme.subText} text-xs text-center mt-2 px-1`}>
                                                  {displayLabel}
                                                </div>
                                                ...
                                                {eventStats.groupStats.length > 0 && (
                                                  <div className="flex items-center gap-3 mt-4">
                                                    <button
                                                      ...
                                                      disabled={eventStats.currentGroupIndex === 0}
                                                    ...
                                                    >
                                                      <span className={`${theme.text} text-lg`}>←</span>
                                                    </button>
                                                    
                                                    <div className="flex gap-1">
                                                      {[...Array(eventStats.groupStats.length + 1)].map((_, idx) => (
                                                        <div
                                                          key={idx}
                                                          className={`w-1.5 h-1.5 rounded-full transition-all ${
                                                            eventStats.currentGroupIndex === idx
                                                              ? 'bg-[#3EB489] w-3'
                                                    ...
                                                    <button
                                                      ...
                                                      disabled={eventStats.currentGroupIndex >= eventStats.groupStats.length}
                                                      className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                                                        eventStats.currentGroupIndex >= eventStats.groupStats.length
                                                          ? 'opacity-30 cursor-not-allowed'
                                                          : 'hover:scale-110'
                                                      }`}
                                                      style={{
      ...
      >
                                  <div className="flex items-center justify-between gap-3 flex-shrink-0 mb-4">
                                      ...
                                      <button 
                                        ...
                                        className="bg-[#6a47fa] text-white rounded-full px-6 py-2 text-sm font-light tracking-wider hover:bg-[#5a3fea] transition-colors"
                                      >
                                        see all
                                      </button>
                                    </div>
                              <div className="flex-1 pr-2">{liveEvents.length > 0 ? liveEvents.slice(0, 3).map((event) => {
                                ...
                                const bannerUrl = event.event_banner ? getImageUrl(event.event_banner) : "https://via.placeholder.com/40?text=Event";
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      ...
                                      />
                                      <div>
                                        <p className={`${theme.text} text-sm font-medium`}>{event.event_name}</p>
                                        <p className={`${theme.subText} text-xs`}>{eventDate}</p>
                                      </div>
                                    </div>
                                    {isSelected && (
                                      <div className="flex items-center gap-3 text-gray-400">
                                        <img src={TicketIcon} alt="Ticket" className={`w-5 h-5 ${isDark ? '' : 'filter brightness-0'}`} />
                                        <img src={LinkIcon} alt="Link" className={`w-5 h-5`} />
                                        <div className={`h-4 w-px ${isDark ? 'bg-gray-600' : 'bg-gray-400'}`}></div>
                                        <img src={SettingIcon} alt="More" className={`w-5 h-5 ${isDark ? '' : 'filter brightness-0'}`} />
                                      </div>
                                    )}
                                  </div>
                                );
                              }) : (
                                <div className={`text-center ${theme.subText} mt-10`}>No live events available.</div>
                              )}
                              </div>
                              </div>
  ...
  <div className="flex items-center justify-between gap-3 flex-shrink-0 mb-4">
    <div className="flex items-center gap-3">
      ...
      <div className={`font-semibold text-base ${theme.text}`}>MY GROUPS</div>
    </div>
    <button 
      onClick={() => navigate('/ticket/groups')}
      className={`border border-[rgba(101,73,184,1)] rounded-full px-6 py-2 text-sm font-light tracking-wider transition-colors ${isDark ? 'text-gray-300 hover:bg-gray-800' : 'text-gray-700 hover:bg-gray-100'}`}
    >
      see all
    </button>
  </div>
...
{groupsWithCount.length > 0 ? (
  <div className="flex gap-4 overflow-x-auto horizontal-scrollbar">
    {groupsWithCount.slice(0, 6).map((group) => {
        const groupImageUrl = group.company_logo ? getImageUrl(group.company_logo) || ProfileImage : ProfileImage;
        ...
        return (
          <div 
            ...
            style={{ boxShadow: isDark ? 'inset 3px 3px 6px rgba(0,0,0,0.5), inset -3px -3px 6px rgba(60,60,60,0.25)' : 'inset 3px 3px 6px rgba(0,0,0,0.1), inset -3px -3px 6px rgba(255,255,255,0.5)' }}
          >
            ...
            <p className={`${theme.text} text-sm font-medium text-center`}>{group.name}</p>
            <p className={`${theme.subText} text-xs text-center`}>
              {totalEvents} {totalEvents === 1 ? 'event' : 'events'} created
            </p>
            <button 
              onClick={() => navigate(`/ticket/group/${group._id}`)}
    ...
    <div className="flex-1 flex items-center justify-center">
      ...
      >
        <img src={PlusIcon} alt="Create Group" className="w-6 h-6" />
      </button>
    </div>
  )}
</div>
