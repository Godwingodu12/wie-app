...
const BankDetails = () => {
  ...
  const fetchBankDetails = async () => {
    ...
    try {
      ...
      if (
        ...
        Array.isArray(response.data.bankDetails)
      ) {
  ...
  const filteredBankDetails = useMemo(() => {
    ...
    if (categoryFilter !== "All") {
      filtered = filtered.filter(
        (detail) => detail.source_category === categoryFilter
      );
    }
    ...
    if (sourceTypeFilter !== "All") {
      filtered = filtered.filter(
        (detail) => detail.source_type === sourceTypeFilter
      );
    }
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col flex-1 md:ml-20 lg:ml-20 overflow-x-hidden">
          {/* Top Header */}
          <header
            className="flex items-center justify-between px-3 md:px-4 lg:px-6 w-full overflow-hidden"
            style={{ height: HEADER_HEIGHT }}
          >
          ...
          <main className="flex-1 p-3 md:p-4 lg:p-6 overflow-y-auto overflow-x-hidden">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="mb-6 md:mb-8">
                <h1
                  className={`text-lg md:text-3xl font-semibold ${theme.text} flex items-center gap-3 mb-4`}
                >
                  Bank Details
                </h1>
                ...
                {summary && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                    ...
                    >
                      <p className={`text-xs ${theme.subText} mb-1`}>
                        Main Events Bank Accounts
                      </p>
                      <p className={`text-2xl font-bold ${theme.text}`}>
                        {summary.from_main_events}
                      </p>
                    </div>
                    <div
                      className={`p-4 rounded-2xl ${theme.cardBg}`}
                    ...
                    >
                      <p className={`text-xs ${theme.subText} mb-1`}>
                        Sub Events Bank Accounts
                      </p>
                      <p className={`text-2xl font-bold ${theme.text}`}>
                        {summary.from_sub_events}
                      </p>
                    </div>
                    <div
                      className={`p-4 rounded-2xl ${theme.cardBg}`}
              ...
              {loading ? (
                <div className="flex items-center justify-center min-h-[500px]">
                  <div className="text-center">
                    ...
                    ></div>
                    <p className={`${theme.subText} mt-4`}>
                ...
                >
                  ...
                  <p
                    className={`${theme.text} text-center text-xl md:text-2xl`}
                  >
                    {searchValue
                      ? "No matching bank details found"
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-3 mb-6">
                    ...
                    >
                      ...
                      <option value="Organization/Group">
                        Organization/Group
                      </option>
                    </select>
                          ...
                          if (!detail) {
                            return (
                              <tr key={`placeholder-${index}`}>
                                <td className="py-8 px-4 min-h-[78px]">
                                  &nbsp;
                                </td>
                            ...
                            >
                              <td className={`py-8 px-4 ${theme.text} text-sm`}>
                                <div>
                                  <div className="font-medium truncate pr-4">
                                    {detail.source_name || "N/A"}
                                  </div>
                                  {detail.parent_name && (
                                    <div
                                      className={`text-xs ${theme.subText} truncate pr-4`}
                                    >
                              ...
                              <td className={`py-8 px-4 ${theme.text} text-sm`}>
                                <div className="truncate pr-4">
                                  {detail.bank_acc_holder || "N/A"}
                                </div>
                              </td>
                              <td
                                className={`py-8 px-4 ${theme.text} text-sm font-mono`}
                              >
                                <div className="truncate pr-4">
                                  {maskAccountNumber(detail.bank_acc_no)}
                                </div>
                              </td>
                              <td
                                className={`py-8 px-4 ${theme.text} text-sm font-mono`}
                              >
                                <div className="truncate pr-4">
                                  {detail.bank_ifsc || "N/A"}
                                </div>
                              </td>
                              <td className={`py-8 px-4 ${theme.text} text-sm`}>
                                <div className="truncate pr-4 capitalize">
                                  {detail.bank_acc_type || "N/A"}
                                </div>
                              </td>
                            </tr>
                          );
                        ...
                        >
                          <div className="flex items-start gap-3 mb-4">
                            ...
                            <div className="min-w-0 flex-1">
                              <p
                                className={`${theme.text} font-semibold text-sm mb-1`}
                              >
                                {detail.source_name}
                              </p>
                              {detail.parent_name && (
                          ...
                          <div className={`space-y-2 text-xs ${theme.subText}`}>
                            <div className="flex justify-between">
                              ...
                              <span className={`${theme.text} truncate ml-2`}>
                                {detail.bank_acc_holder || "N/A"}
                              </span>
                            </div>
                            <div className="flex justify-between">
