...
const selectedDateStyle = { background: 'linear-gradient(147.67deg, #2979FF 13.16%, #9DC1FF 100.03%)' };
 const fullDescription = useMemo(() => eventData?.event_description?.split('\n') || [], [eventData]);
    ...
    const mainEventDateRange = useMemo(() => {
        if (!eventData?.event_dates?.length) return [];
        const { start_date, end_date } = eventData.event_dates[0];
        const range = [];
        let currentDate = new Date(start_date);
        const lastDate = new Date(end_date || start_date);
    ...
    // This is the function you asked about. It determines what content to show based on the active tab.
    const renderContent = () => {
    ...
    // Safe access to event data with fallbacks
                ...
                <div className="max-w-7xl mx-auto space-y-8">
                            ...
                            <div className="flex flex-wrap md:flex-nowrap items-center gap-4">
                                ...
                                <div className="w-full md:w-auto order-first md:order-none flex-shrink-0">
                                    <div className="flex items-center justify-center gap-1 bg-black/20 p-1.5 rounded-full backdrop-blur-sm border border-white/10">
                                        <button className="bg-transparent hover:bg-white/10 text-white font-semibold py-2 px-4 rounded-full flex items-center transition-colors duration-300 text-sm"><HeartIcon /> Like count</button>
                                        <button className="bg-transparent hover:bg-white/10 text-white font-semibold py-2 px-4 rounded-full flex items-center transition-colors duration-300 text-sm"><TicketIcon /> Ticket count</button>
                                        <button className="bg-transparent hover:bg-white/10 text-white font-semibold py-2 px-4 rounded-full flex items-center transition-colors duration-300 text-sm"><ShareIcon /> Share count</button>
                                    </div>
                                </div>
                                <div className="flex items-center flex-1 justify-end">
                                     <button className="bg-white/10 hover:bg-white/20 backdrop-blur-sm p-3 rounded-full transition-colors duration-300">
                                        <EnlargeIcon />
                                    </button>
                                </div>
                            </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-center">
                            <div className="bg-[#1C1C1E] p-1.5 rounded-2xl inline-flex">
