...
const UpdateTicketMedia = () => {
    ...
    const displayAlert = showAlert || ((data) => setAlert({ ...data, show: true }));
    ...
    useEffect(() => {
        if (!initialLoading) {
            ...
            const dataToSave = {
                event_logo: previews.event_logo?.startsWith('data:') ? previews.event_logo : null,
                event_banner: previews.event_banner?.startsWith('data:') ? previews.event_banner : null,
                college_authorisation: previews.college_authorisation?.data?.startsWith('data:') 
                    ? previews.college_authorisation 
                    : null,
                event_images: previews.event_images?.filter(img => 
                    !img.isExisting && img.preview?.startsWith('data:')
                ) || []
            };
            
            // Only save if there's actual new data
            const hasNewData = dataToSave.event_logo || dataToSave.event_banner || 
        ...
        const initializeState = async () => {
            ...
            try {
                ...
                const serverMedia = {
                    ...
                    event_banner: ticket.event_banner ? getUrl(ticket.event_banner) : null,
                    ...
                    event_images: (ticket.event_images || []).map((img, index) => ({
                        ...
                        name: img.originalName || img.path?.split(/[/\\]/).pop() || `image-${index}`,
                        isExisting: true,
                ...
                if (savedStateJSON) {
                    try {
                        ...
                        const mergedPreviews = {
                            ...
                            college_authorisation: savedPreviews.college_authorisation?.data?.startsWith('data:')
                            ...
                            event_images: [
                                ...
                                ...(savedPreviews.event_images || []).filter(img => 
                                    !img.isExisting && img.preview?.startsWith('data:')
                                )
                            ]
                        };
                        ...
                        if (savedPreviews.event_logo?.startsWith('data:')) {
                            newFormData.event_logo = base64ToFile(savedPreviews.event_logo, 'event_logo');
                        }
                        
                        if (savedPreviews.event_banner?.startsWith('data:')) {
                            newFormData.event_banner = base64ToFile(savedPreviews.event_banner, 'event_banner');
                        }
                        
                        if (savedPreviews.college_authorisation?.data?.startsWith('data:')) {
    ...
    const handleMultipleFileChange = async (e) => {
        ...
        if (oversizedFiles.length > 0) {
            setErrors(prev => ({ 
                ...prev, 
                event_images: `One or more files exceed the 50MB limit: ${oversizedFiles.map(f => f.name).join(', ')}` 
            }));
            return;
        }
    ...
    const validateForm = async () => {
        ...
        try {
            ...
            console.error("Could not fetch form progress, performing fallback validation.", err);
            if (!hasBanner) {
                newErrors.general = "Event banner is required to proceed.";
            }
        }
        ...
        if (Object.keys(newErrors).length > 0) {
            ...
            displayAlert({ type: 'error', message: 'Missing Required Files', description: firstError });
            return false;
        }
        return true;
    };
    const handleSubmit = async (e) => {
        ...
        if (formData.event_images && Array.isArray(formData.event_images) && formData.event_images.length > 0) {
            let imageCount = 0;
            formData.event_images.forEach((file, index) => { 
                if (file instanceof File) { 
                    ...
                    hasNewFiles = true;
                    imageCount++;
                    console.log(`Appending event_images[${index}]: ${file.name} (${file.type}, ${(file.size / 1024 / 1024).toFixed(2)}MB)`);
                } else {
                    console.log(`Skipping event_images[${index}]: Not a File object`, typeof file);
                }
            });
        ...
        if (!hasNewFiles) {
            console.log('No new files to upload. Checking if existing media meets requirements...');
            try {
                ...
                <main className="flex-1 relative p-4 sm:p-6 md:p-8 overflow-y-auto">
                    ...
                    darkMode
                      ...
                      : "bg-[#1E1242] text-gray-300"
                            ...
                            <p className="text-black dark:text-gray-400">Drop your visuals here to bring your event to life.</p>
                        </header>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-12 ">
                            {isEducationalOrg && (
                                <FileInput 
                                    ...
                                    preview={previews.college_authorisation?.data || previews.college_authorisation?.url || previews.college_authorisation} 
                            ...
                            <div>
                                <label className="flex items-center text-sm font-medium text-black dark:text-gray-400 mb-2">
                                    Images & Videos <InfoTooltip note="Optional. Max 10 files total, max 1 video. Supported: JPG, JPEG, PNG, GIF, WEBP, MP4, AVI, MOV, WMV, FLV, WEBM." />
                                </label>
                                ...
                                >
                                    ...
                                    <span className="text-indigo-500 dark:text-indigo-400 font-semibold">
                                        {previews.event_images && previews.event_images.length > 0 ? 'Add more files...' : 'Upload images & videos'}
                                    </span>
                                    <span className="text-xs text-gray-600 dark:text-gray-200">
                                        {previews.event_images ? previews.event_images.length : 0}/10 files • Max 1 video
                                    </span>
                                    <input 
