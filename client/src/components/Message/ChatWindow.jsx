...
const ChatWindow = ({ chat, onBack, isDark }) => {
  ...
  const handleContextMenu = (e, messageId) => {
    ...
    if (!message || !message.isSender) return;
  ...
  const showAlert = (type, message, description) => {
    setAlertConfig({ type, message, description, show: true });
  };

  const hideAlert = () => {
    setAlertConfig((prev) => (prev ? { ...prev, show: false } : null));
  };

  const showModal = (title, message, action, onConfirmCallback) => {
    setModalConfig({
      title,
      message,
      action,
      ...
      if (data.chatId === chat._id) {
        const transformedMessage = {
          ...
          isSender:
            ...
            data.message.sender === currentUserId,
          pending: false,
        };
    ...
    const handleChatDeleted = (data) => {
      if (data.chatId === chat._id) {
        window.dispatchEvent(
          new CustomEvent("chat-deleted-by-user", {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const socketMessageArrived = messagesRef.current?.some(
        (msg) => msg._id.toString() === realMessageId && !msg.pending
      );
  ...
  const handleClearChat = () => {
    ...
    showModal(
      "Clear Chat",
      "Are you sure you want to clear all messages? This will only clear the chat history for you.",
      "clear",
      async () => {
        closeModal();
        try {
          ...
          showAlert(
            "success",
            "Chat Cleared",
            "Chat cleared successfully (only for you)."
          );
        } catch (error) {
  ...
  const handleDeleteChat = () => {
    ...
    showModal(
      "Delete Chat",
      "Are you sure you want to delete this chat? This will remove the chat from your list, but not for the other participant.",
      "delete", // Action type for button styling (optional)
      async () => {
        closeModal(); // Close the modal before starting the async operation
        try {
          await deleteChat(chat._id); // Notify parent to remove from list and go back
          window.dispatchEvent(
            new CustomEvent("chat-deleted-by-user", {
              detail: { chatId: chat._id },
            })
          ...
          showAlert(
            "success",
            "Chat Removed",
            "Chat successfully removed from your list."
          );
        } catch (error) {
  ...
  const handleClearAndDeleteChat = () => {
    setShowOptionsMenu(false);
    showModal(
      "Permanent Chat Deletion",
      "Are you sure you want to permanently delete this chat? All messages will be deleted from the database for BOTH participants. This action cannot be undone.",
      "delete",
      async () => {
        closeModal();
        try {
  ...
  const getMessageStatus = (message) => {
    ...
    const isRead =
      message.isRead ||
      message.readBy?.some((id) => id.toString() === receiverId);
    if (isRead) {
    ...
    <div className="flex flex-col w-full h-full overflow-hidden">
      ...
      >
        ...
        >
          ...
          {isConnected &&
            onlineUsers?.has(chat.participant?._id || chat.participant?.id) && (
        ...
        <div className="flex-1 min-w-0">
          <button
            onClick={() => {
              const participantId =
                chat.participant?._id || chat.participant?.id;
              if (participantId) {
                navigate(`/profile/${participantId}`);
              }
            }}
      ...
      >
        {loading && messages.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3">
            <p style={{ color: theme.subText }}>{fetchError}</p>
            <button
                    ...
                    <div className="flex items-end gap-2">
                      ...
                      {isSender && (
                        <button
                          ...
                          className={`p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/20 mb-2 transition-opacity ${
      ...
      {modalConfig && (
        <Modal
          ...
          darkMode={isDark} // Pass the dark mode prop
        />
      )}
      <Alert alert={alertConfig} onClose={hideAlert} darkMode={isDark} />
    </div>
  );
};
