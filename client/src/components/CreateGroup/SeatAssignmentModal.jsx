...
import ScrollBarStyles from "../../components/ScrollBarStyle.jsx";

import React, { useState, useEffect } from "react";
import darkThemeStyles from "./darkThemeStyles.jsx";
import lightThemeStyles from "./lightThemeStyles.jsx";
const SeatAssignmentModal = ({
  ...
  const getTicketTypeColorLocal = (typeId) => {
    ...
    if (typeof getTicketTypeColor === "function")
      return getTicketTypeColor(typeId);
    ...
    if (idx === -1) {
      ...
      return ticketTypeColors.length
        ? ticketTypeColors[
            Math.abs(hashString(typeId)) % ticketTypeColors.length
          ]
        : "#6B7280";
    }
    return ticketTypeColors.length
      ? ticketTypeColors[idx % ticketTypeColors.length]
      : ticketTypes[idx].color || "#6B7280";
  };
  ...
  const getAssignedTypeIdForSeat = (seatId) => {
    const found = Object.entries(assignments || {}).find(
      ([typeId, seatIds]) => Array.isArray(seatIds) && seatIds.includes(seatId)
    );
  ...
  const handleRowClick = (rowLabel) => {
    ...
    if (selectionMode === "checkbox") {
      ...
      if (!unassignedInRow.length) {
        setErrors({
          general: "All seats in this row are already assigned to other types",
        });
        return;
      }

      const newSelected = new Set(selectedSeats);
      const allSelected = unassignedInRow.every((s) =>
    ...
    const unassignedInRow = rowSeats.filter(
      (s) => !getAssignedTypeIdForSeat(s.seatId)
    );
    if (!unassignedInRow.length) {
    ...
    if (unassignedInRow.length > remaining) {
      setErrors({
        general: `Cannot assign ${unassignedInRow.length} seats. Only ${remaining} remaining for this ticket type.`,
      });
      return;
    }
  ...
  const handleSeatClick = (seat) => {
    ...
    if (selectionMode === "single") {
      ...
      if (currentAssignment === selectedTicketType) {
        newAssignments[selectedTicketType] = (
          newAssignments[selectedTicketType] || []
        ).filter((id) => id !== seat.seatId);
        if (newAssignments[selectedTicketType].length === 0)
          delete newAssignments[selectedTicketType];
      ...
      if (currentAssignment && currentAssignment !== selectedTicketType) {
        setErrors({
          general:
            "Seat already assigned. Remove from other ticket type first.",
        });
        return;
      }
      ...
      console.log(
        `💰 Assigning seat ${seat.seatId} with price: ₹${ticketPrice}`
      );
      // check capacity
      const remaining = getRemainingCapacity(selectedTicketType);
      if (remaining <= 0) {
    ...
    if (selectionMode === "checkbox") {
      ...
      if (currentAssignment && currentAssignment !== selectedTicketType) {
        setErrors({
          general: "Cannot select seats assigned to other ticket types",
        });
        return;
      }
  ...
  const applyCheckboxSelection = () => {
    ...
    if (selectedArray.length > remaining) {
      setErrors({
        general: `Cannot assign ${selectedArray.length} seats. Only ${remaining} remaining for this ticket type.`,
      });
      return;
    }
  ...
  const applyRangeSelection = () => {
    ...
    if (rangeSeats.length > remaining) {
      setErrors({
        general: `Cannot assign ${rangeSeats.length} seats. Only ${remaining} remaining for this ticket type.`,
      });
      return;
    }
    ...
    console.log(
      "✅ Applied range selection with color for:",
      selectedTicketType
    );
    console.log("Color:", getTicketTypeColorLocal(selectedTicketType));
  ...
  const handleSave = () => {
    ...
    ticketTypes.forEach((ticket) => {
      ...
      if (assigned > ticket.capacity) {
        validationErrors[
          ticket.id
        ] = `Too many seats assigned (${assigned}/${ticket.capacity})`;
      }
    });
    ...
    if (Object.keys(validationErrors).length > 0) {
      setErrors({
        ...validationErrors,
        general: "Some ticket types exceed their capacity",
      });
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-[##212426] bg-opacity-75 flex items-center justify-center p-2">
      <ScrollBarStyles isDark={isDark} />

      <div
        className={`w-full h-full max-w-[98vw] max-h-[98vh] rounded-lg overflow-hidden flex flex-col ${
          isDark ? "bg-[#212426]" : "bg-[#FEFEFE]"
        }`}
      >
        {/* Header */}
        <div
          className={`p-4 ${isDark ? "border-gray-700" : "border-gray-200"}`}
        ></div>
        ...
        <div className="text-center">
          ...
          <h1
            className={`text-xs md:text-xs lg:text-sm font-light text-gray-500`}
          >
            Arrange your complete event layout here.{" "}
          </h1>
        </div>

        {/* Content */}
        <div
          className="flex-1 my-auto p-4 overflow-y-auto custom-scrollbar"
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                    ...
                    return (
                      <div key={ticket.id} className="relative">
                        <button
                          ...
                          className={`w-full p-2.5 rounded-lg transition-all text-left 
              ${
                selectedTicketType === ticket.id
                  ? isDark // Selected State
                    ? "bg-gray-800"
                    : "bg-[#FFFFFF]" // Light Mode Selected: White
                  : isDark // Unselected State
                  ? "bg-[#424242] border-gray-600 hover:border-gray-500"
                  : "bg-[#F5F5F5] border-gray-300 hover:border-gray-400" // Light Mode Unselected: F5F5F5
              }`}
                        >
                          <div className="flex items-center gap-2">
                            <div
                              className="w-5 h-5 rounded flex-shrink-0 border-2 border-white"
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              >
                                {assigned}/{ticket.capacity} seats{" "}
                                {isComplete && (
                        ...
                        {assigned > 0 && (
                          <button
                            onClick={() =>
                              removeTicketTypeAssignments(ticket.id)
                            }
                            className="absolute top-1 right-1 p-1 bg-red-500 hover:bg-red-600 text-white rounded text-xs transition"
            ...
            <div className="lg:col-span-2">
              <div className={`rounded-lg  p-4 overflow-auto `}>
                ...
                <div
                  ...
                  className="space-y-2 max-h-[500px] transition-transform duration-200 "
                ...
                >
                  {seatsByRow.map((rowSeats, rowIndex) => {
                    ...
                    const centerOffset = isShapedLayout
                      ? `${((maxSeatsInRow - seatsInRow) / 2) * 42}px`
                      : "0px";

                    // For visuals: check if whole row assigned to the selected ticket type
                    const rowAssignedSeatsForCurrentType = rowSeats.filter(
                      (s) =>
                        getAssignedTypeIdForSeat(s.seatId) ===
                        selectedTicketType
                    );
                    const isRowFullyAssigned =
                      rowAssignedSeatsForCurrentType.length ===
                        rowSeats.length && rowSeats.length > 0;
                      ...
                      <div key={rowLabel} className="flex items-center gap-2">
                        <div className="flex items-center gap-2 flex-shrink-0">
                          {selectionMode === "checkbox" && (
                            <input
                              ...
                              checked={rowSeats.every((seat) => {
                                const assigned = getAssignedTypeIdForSeat(
                                  seat.seatId
                                );
                                return (
                                  assigned === selectedTicketType ||
                          ...
                          <button
                            onClick={() =>
                              selectionMode !== "checkbox" &&
                              handleRowClick(rowLabel)
                            }
                            className={`w-7 text-center font-bold  text-sm cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900 rounded transition ${
                        ...
                        >
                          {rowSeats.map((seat) => {
                            const assignedTypeId = getAssignedTypeIdForSeat(
                              seat.seatId
                            );
                            const assignedColor = assignedTypeId
                              ? getTicketTypeColorLocal(assignedTypeId)
                              : null;
                            const isAssigned = !!assignedTypeId;
                            const isCurrentType =
                              assignedTypeId === selectedTicketType;
                            const isInCheckboxSelection = selectedSeats.has(
                              seat.seatId
                            );
                            const isRangeStart =
                              rangeStart?.seatId === seat.seatId;
                            const isRangeEnd = rangeEnd?.seatId === seat.seatId;

                            // in-range detection
                            const seatIndex = seatingLayout.seats.findIndex(
                              (s) => s.seatId === seat.seatId
                            );
                            const startIndex = rangeStart
                              ? seatingLayout.seats.findIndex(
                                  (s) => s.seatId === rangeStart.seatId
                                )
                              : -1;
                            const endIndex = rangeEnd
                              ? seatingLayout.seats.findIndex(
                                  (s) => s.seatId === rangeEnd.seatId
                                )
                              : -1;
                            const isInRange =
                              rangeStart &&
                              rangeEnd &&
                              seatIndex >= Math.min(startIndex, endIndex) &&
                              seatIndex <= Math.max(startIndex, endIndex);
                            ...
                            if (isAssigned) {
                              ...
                              const previewHex =
                                getTicketTypeColorLocal(selectedTicketType);
                            ...
                            const borderStyle = isRangeStart
                              ? "3px solid #FCD34D"
                              : isRangeEnd
                              ? "3px solid #FB923C"
                              : isAssigned
                            ...
                            return (
                              <button
                                ...
                                style={{
                                  backgroundColor: (() => {
                                    ...
                                    const assignedTypeId =
                                      getAssignedTypeIdForSeat(seat.seatId);

                                    if (assignedTypeId) {
                                      const color =
                                        getTicketTypeColorLocal(assignedTypeId);
                                      console.log(
                                        `✅ Seat ${seat.seatId} assigned to ${assignedTypeId} with color ${color}`
                                      );
                                ...
                                title={
                                  isRangeStart
                                    ? `Range Start: ${seat.seatId}`
                                    : isRangeEnd
                                    ? `Range End: ${seat.seatId}`
                                    : isAssigned
                                    ? (() => {
                                        const ticket = ticketTypes.find(
                                          (t) => t.id === assignedTypeId
                                        );
                                        const price =
                                          ticket?.price ||
                                          ticket?.ticket_price ||
                              ...
                              >
                                ...
                                {isRangeStart && (
                                  <div className="absolute -top-2 -left-2 bg-yellow-500 text-white text-[10px] font-bold px-1 rounded shadow">
                                    START
                                  </div>
                                )}
                                {isRangeEnd && (
                                  <div className="absolute -bottom-2 -right-2 bg-orange-500 text-white text-[10px] font-bold px-1 rounded shadow">
                                    END
                                  </div>
                                )}
                              </button>
            ...
            <div
              className={`p-3 my-auto rounded-[12px] ${
                isDark ? "bg-[#323436]" : "bg-[#F1F1F1] border border-gray-300"
              }`}
            >
              <div>
                <h3
                  className={`text-sm mb-2 ${
                    isDark ? "text-white" : "text-gray-900" // CHANGED: Use isDark state
                  }`}
                >
                  Selection Mode
                </h3>
                  ...
                  {[
                    {
                      mode: "single",
                      label: "Single",
                      desc: "Click seats individually",
                    },
                    {
                      mode: "checkbox",
                      label: "Multiple",
                      desc: "Select then apply",
                    },
                    ...
                    >
                      ...
                      <p
                        className={`text-xs ${
                          isDark ? "text-gray-400" : "text-gray-600" // CHANGED: Use isDark state
                        }`}
                      >
                        {desc}
                      </p>
              ...
              <button
                ...
                className="w-full mt-2 px-3 py-1.5 text-sm border border-red-500 text-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition"
              >
                Clear All
              </button>
            </div>
        ...
        {/* Zoom controls */}
        <div
          className={`flex items-center mx-auto rounded-[8px]  mb-3 p-1 ${
            isDark ? "bg-gray-700 text-white" : "bg-gray-200 text-gray-700" // CHANGED: Used isDark
          }`}
        >
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                ...
                const cur = parseFloat(
                  c.style.transform?.match(/scale\(([\d.]+)\)/)?.[1] || "1"
                );
              ...
              className={`p-3 rounded-lg transition ${
                isDark // CHANGED: Used isDark
                  ? "bg-gray-700 hover:bg-gray-600 text-white"
                  : "bg-gray-200 hover:bg-gray-300 text-gray-700"
              }`}
              title="Zoom Out"
            >
              <img src={ZoomOut} alt="" className={isDark ? "" : "invert"} />
            </button>
            <button
              onClick={() => {
                const c = document.getElementById("seat-map-container");
              ...
              title="Zoom In"
            >
              <img src={ZoomIn} alt="" className={isDark ? "" : "invert"} />
            </button>
            <button
              onClick={() => {
                const c = document.getElementById("seat-map-container");
              ...
              title="Zoom Reset"
            >
              <img src={ZoomRest} alt="" className={isDark ? "" : "invert"} />
            </button>
            <button
              onClick={onClose}
              title="Close"
        ...
        <div
          className={`flex-shrink-0 p-4 border-t ${
            isDark
              ? "border-gray-700 bg-[#212426]" // CHANGED: Used isDark
              : "border-gray-300 bg-white" // CHANGED: Used isDark
          }`}
        >
            ...
            {(Object.keys(assignments).length > 0 ||
              ...
              <div className="pb-3 border-b border-gray-300 dark:border-gray-600">
                ...
                <div className="flex flex-wrap gap-3 justify-center items-center">
                  {ticketTypes.map((ticket) => {
                    ...
                    if (String(selectedTicketType) === String(ticket.id)) {
                      if (selectionMode === "checkbox") {
                        ...
                        pendingCount = Array.from(selectedSeats).filter(
                          (seatId) => {
                            const currentAssignment =
                              getAssignedTypeIdForSeat(seatId);
                            return (
                              !currentAssignment ||
                              currentAssignment === ticket.id
                            );
                          }
                        ).length;
                      } else if (
                        ...
                        const seats = seatingLayout.seats;
                        const startIdx = seats.findIndex(
                          (s) => s.seatId === rangeStart.seatId
                        );
                        const endIdx = seats.findIndex(
                          (s) => s.seatId === rangeEnd.seatId
                        );
                        const [from, to] =
                          startIdx < endIdx
                        ...
                        pendingCount = seats.slice(from, to + 1).filter((s) => {
                          const currentAssignment = getAssignedTypeIdForSeat(
                            s.seatId
                          );
                          return (
                            !currentAssignment ||
                  ...
                  {ticketTypes.every((t) => getAssignedCount(t.id) === 0) &&
                    selectedSeats.size === 0 &&
                    !rangeStart && (
                      <p className="text-sm text-gray-500 dark:text-gray-400 italic">
                        No seats assigned yet. Select a ticket type and click
                        seats to begin.
                      </p>
                    )}
                </div>
