...
const RecordedDatePickerModal = ({ isOpen, onClose, onSave, initialDates, initialDateType, darkMode, showAlert, minDate,
    maxDate }) => {
    ...
    const handleDateClick = (dateStr) => {
        ...
        if (dateType === 'one-day') {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                if (!isDisabled && date.getDay() === clickedDayOfWeek) {
                    const dateStrFormat = format(date, "yyyy-MM-dd");
                    if (!newSelection.includes(dateStrFormat) && !allConfiguredDates.includes(dateStrFormat)) {
                        newSelection.push(dateStrFormat);
                    }
                }
            }
            setCurrentSelection(newSelection.sort());
        } else { // Multi days or Recurring
    ...
    const handleAddOrUpdate = () => {
        if (currentSelection.length === 0) {
            showAlert({ type: 'error', message: 'No Date Selected', description: 'Please select at least one date from the calendar.' });
            return;
        }

        if (!videoLink.trim() && !videoFile) {
            showAlert({ type: 'error', message: 'Video Source Missing', description: 'Please provide either a video link or attach a video file.' });
            return;
        }
    ...
    const handleSaveChanges = () => {
        ...
        let actualDateType = dateType;
        if (actualDateType === 'one-day' && flattenedDates.length > 1) {
            actualDateType = 'multi-day';
        } else if (actualDateType === 'multi-day' && flattenedDates.length === 1) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                <div
                    ...
                    className={`flex items-center justify-center h-10 w-10 rounded-lg transition-colors ${isDisabled
