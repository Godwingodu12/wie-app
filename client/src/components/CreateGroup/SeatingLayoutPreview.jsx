...
const SeatingLayoutPreview = ({ seatingLayout, onSeatSelect, darkMode = false, isExpandable = false,ticketTypeAssignments = []  }) => {
  ...
  if (!seatingLayout || !seatingLayout.seats || seatingLayout.seats.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500 dark:text-gray-400 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
        ...
        <p className="text-xs mt-1">Upload a file and generate the layout first</p>
      </div>
    );
  }
  ...
  const handleSeatClick = (seat) => {
    ...
    if (onSeatSelect) {
      onSeatSelect(Array.from(newSelected).map(id => 
        seats.find(s => s.seatId === id)
      ));
    }
  };
const getSeatColor = (seat) => {
  // STEP 1: Check for saved color (from database)
  if (seat.ticketTypeColor) {
    return seat.ticketTypeColor;
  }
  
  // STEP 2: Fallback to assignments lookup
  if (seat.ticketTypeId && ticketTypeAssignments && ticketTypeAssignments.length > 0) {
    const assignment = ticketTypeAssignments.find(a => 
...
const content = (
  <div 
    ref={containerRef}
    className={`relative w-full overflow-hidden bg-gray-50 dark:bg-[#1a1a1a] ${
      isExpanded 
  ...
  >
    ...
    {detectionMethod && (
      <div className="absolute top-2 left-2 z-20 bg-blue-600 text-white px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
        ...
        {detectionMethod === 'pattern_recognition' && 'Auto-detected from image'}
    ...
    <div className={`absolute ${isExpanded ? 'top-20 right-4' : 'top-2 right-2'} z-20 flex gap-2`}>
      <button
        ...
        className={`bg-white dark:bg-gray-800 p-2 rounded shadow hover:bg-gray-100 dark:hover:bg-gray-700 ${
          isExpanded ? 'text-white bg-gray-800 hover:bg-gray-700' : ''
        }`}
      ...
      {isExpanded && (
        <div className="bg-gray-800 text-white px-3 py-2 rounded shadow flex items-center gap-2">
    ...
    <div className="absolute top-12 left-0 right-0 z-10">
      <div className="bg-gradient-to-b from-gray-800 to-gray-700 text-white text-center py-2 mx-4 rounded shadow-lg">
        <div className="text-xs font-semibold tracking-wider">STAGE / SCREEN</div>
        <div className="mt-1 h-1 w-24 mx-auto bg-yellow-400 rounded"></div>
      </div>
    </div>
    {/* Seating area*/}
      ...
      >
        <div className="space-y-2">
          {seatsByRow.map((rowSeats, rowIndex) => {
            ...
            const maxSeatsInLayout = Math.max(...seatsByRow.map(r => r.length));
            const seatsInThisRow = rowSeats.length;
            
            // Detect if this is a shaped layout
            const isShapedLayout = layoutStyle && ['circular', 'oval-horizontal', 'oval-vertical', 'triangular-down', 'triangular-up', 'curved', 'image_detected'].includes(layoutStyle);
            
            // Center offset for non-rectangular layouts
            const centerOffset = isShapedLayout 
              ? `${((maxSeatsInLayout - seatsInThisRow) / 2) * (seatSize + 4)}px`
              : '0px';
                  ...
                  {rowSeats.map((seat) => {
                    ...
                    return (
                      <button
                        ...
                        } ${!seat.isAvailable && !seat.ticketTypeColor ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}
                        style={{
                          width: `${seatSize}px`,
                          height: `${seatSize}px`,
                          backgroundColor: seatColor,
                          color: '#FFFFFF', // Always white text for visibility
                          border: seat.ticketTypeColor ? '2px solid rgba(255,255,255,0.6)' : 'none',
                          boxShadow: seat.ticketTypeColor ? '0 2px 6px rgba(0,0,0,0.3)' : 'none'
                        }}
                        title={`${seat.seatId}${seat.ticketTypeName ? ` - ${seat.ticketTypeName}` : ''}`}
                        disabled={!seat.isAvailable && !seat.ticketTypeColor}
                      >
                        <svg className={`mx-auto ${seatSize > 40 ? 'w-6 h-6' : 'w-4 h-4'}`} fill="currentColor" viewBox="0 0 24 24">
    ...
    <div className={`absolute bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-300 dark:border-gray-700 ${
      isExpanded ? 'p-4' : 'p-3'
    }`}>
      <div className="space-y-2">
        ...
        {ticketTypeAssignments && ticketTypeAssignments.length > 0 && (
          <div className="flex gap-3 justify-center items-center flex-wrap pb-2 border-b border-gray-200 dark:border-gray-700">
            {ticketTypeAssignments.map((assignment, idx) => (
              <div key={idx} className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-600">
                <div 
                  className="w-5 h-5 rounded border-2 border-white shadow-sm flex-shrink-0"
                  style={{ backgroundColor: assignment.color }}
                ></div>
                <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">
                  {assignment.ticketTypeName}: {assignment.assignedSeats?.length || 0}/{assignment.capacity}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="text-center text-xs text-gray-600 dark:text-gray-400 pt-1">
          ...
          {layoutStyle && layoutStyle !== 'grid' && ` • ${layoutStyle.replace('-', ' ')}`}
        </div>
      </div>
    </div>
    {/* Selected seat info*/}
