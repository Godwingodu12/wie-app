...
const DatePickerModal = ({ isOpen, onClose, onSave, initialDates, darkMode, showAlert, minDate, maxDate }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            // 1. Convert 24-hour initial dates back to 12-hour format for display
            const formattedDates = dates.map(d => {
                // Check if conversion is needed (i.e., startAmPm/endAmPm fields are missing from the raw data)
                const needsConversion = !d.startAmPm || !d.endAmPm;

                if (!needsConversion) return d;
                
                const start = convertTo12Hour(d.startTime);
                const end = convertTo12Hour(d.endTime);
            ...
            setCurrentMonth(dates.length > 0 ? new Date(dates[0].date) : new Date());
            
            // 2. Determine if useSameTime flag should be checked/unchecked
            let allSameTime = true;
            if (formattedDates.length > 1) {
                const first = formattedDates[0];
                allSameTime = formattedDates.every(d => 
            ...
            // --- CONSOLE LOGGING START ---
            console.log("Formatted Dates (State):", formattedDates);
