    ...
    const initializeWebGL = async () => {
      ...
      try {
        const renderer = new Renderer({
          ...
          alpha: true,
          antialias: false, // Disable for better performance on low-end devices
  ...
  gl_FragColor = color;
        ...
        const updatePlacement = () => {
          ...
          resizeTimeoutRef.current = setTimeout(() => {
            ...
            const minWidth = Math.max(wCSS, 320);
            const minHeight = Math.max(hCSS, 568);
            
            renderer.setSize(minWidth, minHeight);

            const dpr = renderer.dpr;
            const w = minWidth * dpr;
            const h = minHeight * dpr;
        ...
        if (containerRef.current) {
          containerRef.current.style.background = `linear-gradient(180deg, ${raysColor}20 0%, transparent 50%)`;
        }
      }
    };
